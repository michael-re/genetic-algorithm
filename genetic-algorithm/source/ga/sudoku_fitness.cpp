...
auto ga::sudoku_fitness::how_fit(const sudoku& puzzle) -> int
{
    ...
    for (auto i = 0, origin_x = 0, origin_y = 0; i < sudoku::detail::grid_h; i++)
    {
        fitness += row_fitness(puzzle, i);
        fitness += col_fitness(puzzle, i);
        fitness += sub_fitness(puzzle, origin_x, origin_y);

        origin_y += sudoku::detail::sub_grid_h;
        if (origin_y == sudoku::detail::grid_h)
        {
            origin_x += sudoku::detail::grid_w;
            origin_y  = 0;
        }
    }
